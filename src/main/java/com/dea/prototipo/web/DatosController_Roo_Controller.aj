...
privileged aspect DatosController_Roo_Controller {
    ...
    public String DatosController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "sortFieldName", required = false) String sortFieldName, @RequestParam(value = "sortOrder", required = false) String sortOrder, Model uiModel) {
    ...
    public String DatosController.delete(@PathVariable("id") Long id, @RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
        Datos datos = Datos.findDatos(id);
        datos.remove();
        uiModel.asMap().clear();
    ...
    void DatosController.addDateTimeFormatPatterns(Model uiModel) {
        uiModel.addAttribute("datos_fechaderegistro_date_format", DateTimeFormat.patternForStyle("M-", LocaleContextHolder.getLocale()));
    }
    
    void DatosController.populateEditForm(Model uiModel, Datos datos) {
        uiModel.addAttribute("datos", datos);
        addDateTimeFormatPatterns(uiModel);
        uiModel.addAttribute("bodegas", Bodega.findAllBodegas());
    }
    
    String DatosController.encodeUrlPathSegment(String pathSegment, HttpServletRequest httpServletRequest) {
